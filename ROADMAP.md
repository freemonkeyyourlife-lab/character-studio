...
| 1. Creation | Character Creator, universal drag and drop, provider registry/router and comparison center, image/video/voice editing | Existing character and media foundations; availability endpoint, automatic image routing with at most one fallback, and a manual image-provider comparison page. Quality scoring, cost tracking and broader routing remain open. |
| 2. Intelligence | Persistent multi-turn chat, layered context, memory, personal AI | Supabase-backed Venice chat merged in PR #1; conversations schema deployed to the selected Supabase project; manually curated character memories are being added. Signed-in live chat/provider verification remains open. |
...
1. Confirm server-only `VENICE_API_KEY` and an available `VENICE_CHAT_MODEL` in the intended deployment.
2. Test a signed-in two-turn conversation, reload it, verify character context and owner isolation, then inspect provider errors and usage. The selected Supabase project had no users at schema deployment, so this live flow was not yet possible.
3. Test saved character memories in a signed-in chat. Plan automated extraction and story state after the basic round trip is verified.
